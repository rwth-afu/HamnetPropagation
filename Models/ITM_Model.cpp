...
// ******Modified in several ways by Hubertus Munz, Aachen, Germany in 9/2014*******
...
//Diffraction
double  ITM::adiff( double d, prop_type &prop, propa_type &propa)
...
//Troposcatter
double  ITM::ascat( double d, prop_type &prop, propa_type &propa)
{ complex<double> prop_zgnd(prop.zgndreal,prop.zgndimag);
...
//Initialiesierung von  Ausbreitungsparametern k,sigma,Impedanz...
...
//Line-of-Sight
double  ITM::alos( double d, prop_type &prop, propa_type &propa)
{ complex<double> prop_zgnd(prop.zgndreal,prop.zgndimag);
  double &wls= this->wls;
  complex<double> r;
  double s, sps, q;
...
//Calculation of Aref
void ITM::lrprop (double d,
          prop_type &prop, propa_type &propa)
{
  bool &wlos=this->wlos;
  bool &wscat=this->wscat;
  ...
  if(prop.mdp!=0)
    {
      ...
      q=this->adiff(0.0,prop,propa);
	  xae=pow(prop.wn*pow(prop.gme,2),-THIRD);
	  d3=mymax(propa.dlsa,1.3787*xae+propa.dla);
	  d4=d3+2.7574*xae;
      a3=this->adiff(d3,prop,propa);
      a4=this->adiff(d4,prop,propa);
	  ...
	  if(!wlos)
	    {
            ...
            a1=this->alos(d1,prop,propa);
			wq=false;
			if(d0<d1)
				{
                    a0=this->alos(d0,prop,propa);
					q=log(d2/d0);
  ...
  if(prop.dist<=0.0 || prop.dist>=propa.dlsa)
    { if(!wscat)
	    {
          q=this->ascat(0.0,prop,propa);
		  d5=propa.dla+200e3;
		  d6=d5+200e3;
          a6=this->ascat(d6,prop,propa);
          a5=this->ascat(d5,prop,propa);
		  if(a5<1000.0)
...
//In this funtion the statistical attenuation is added
...
{ bool wq;
  ...
  if(np>=2)
    { sa=0.0;
	  sb=prop.dist;
	  wq=true;
	  // Testing the Path from each station on
	  for(int i=1;i<np;i++)
	    {     sa+=xi;
		  sb-=xi;
		  //prop.gme is the faktor of the earth's curvature!, qc=0.5*gme!
		  //prop.the is the Steigung beteen the two stations
...
//Linear least square fit, page 28 in ITM notification
...
{ int np, j;
  ...
  prop.terrain_height[0] = pfl[2];
  prop.terrain_height[1] = pfl[np+2];

  //hzns(pfl,prop);-> put into initialize_parameters


  for(j=0;j<2;j++)
    xl[j]=mymin(15.0*prop.hg[j],0.1*prop.dl[j]);
  xl[1]=prop.dist-xl[1];
  prop.dh=d1thx(prop,pfl,xl[0],xl[1]);
  if(prop.dl[0]+prop.dl[1]>1.5*prop.dist)
    { z1sq1(prop,pfl,xl[0],xl[1],za,zb);
	  prop.he[0]=prop.hg[0]+FORTRAN_DIM(pfl[2],za);
	  prop.he[1]=prop.hg[1]+FORTRAN_DIM(pfl[np+2],zb);
      for(j=0;j<2;j++)

      //Definition of radio horizon without obstructions
    ...
    { z1sq1(prop,pfl,xl[0],0.9*prop.dl[0],za,q);
          z1sq1(prop,pfl,prop.dist-0.9*prop.dl[1],xl[1],q,zb);
	  prop.he[0]=prop.hg[0]+FORTRAN_DIM(pfl[2],za);
	  prop.he[1]=prop.hg[1]+FORTRAN_DIM(pfl[np+2],zb);
	}
  prop.mdp=-1;
  ...
  // _>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Aufruf von lrprop bzw. Berechnung von Aref
}
...
//********************************************************
double ITM::point_to_point(double elev[], propv_type &propv, prop_type &prop)

        // deleted double & dbloss as input parameter;
	...
	// conf, rel: .01 to .99 (reliability and confidence)
    // elev[]: [num points - 1], [delta dist(meters)], [height(meters) point 1], ..., [height(meters) point n]
{
    ...
    zc = qerfi(prop.conf);
    zr = qerfi(prop.rel);
    qlrps(prop.Ns,elev,prop.polarization,prop.eps_dielect,prop.conductivity,prop);


    this->lrprop(0.0,prop,propa);
    fs = 32.45 + 20.0 * log10(prop.f_MHz) + 20.0 * log10(prop.distance_km);

  double dbloss = avar(zr,0.0,zc,prop,propv) + fs;
  return dbloss;
}
